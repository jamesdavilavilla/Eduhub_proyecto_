...
const ClassForm = ({
  ...
  const onSubmit = async (formData: ClassSchema) => {
    const actionFn = type === "create" ? createClass : updateClass;
    const result = await actionFn({ success: false, error: false }, formData);
    setState(result);
  };
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={handleSubmit(onSubmit)}>
      <h1 className="text-xl font-semibold">{type === "create" ? "Crear una nueva clase" : "Actualizar clase"}</h1>
